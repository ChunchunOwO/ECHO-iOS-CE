...
test('the app boots the native core and keeps playback mutations ordered', async () => {
  ...
  assert.match(payload, /option\("history", localized\("History", "??"\)\)/u);
  ...
  assert.match(player, /Label\(model\.language == "en" \? "Signal path" : "????"/u);
  ...
  assert.match(player, /"Signal theater" : "????"/u);
  assert.match(player, /"Full chain" : "????"/u);
  assert.match(player, /"Signal doctor" : "????"/u);
  assert.match(player, /"Route flight recorder" : "???????"/u);
  ...
  assert.match(player, /"DAC capability atlas" : "DAC ????"/u);
  assert.match(player, /"Observed" : "???"/u);
  assert.match(player, /"Remote reported" : "????"/u);
  ...
  assert.match(player, /case nil: return english \? "Resampling: \?" : "????\?"/u);
  ...
  assert.doesNotMatch(player, /Label\(model\.language == "en" \? "Playback output" : "????", systemImage: "airplayaudio"\)/u);
  assert.match(player, /Text\(albumLabel\)/u);
  assert.doesNotMatch(player, /Text\(model\.language == "en" \? "NOW PLAYING" : "????"\)/u);
  ...
  assert.match(pages, /"Default order" : "????"[\s\S]*\.tag\("default"\)/u);
  ...
  assert.match(desktopLyrics, /currentLyric\.isEmpty[\s\S]*??????/u);
