...
test('the app boots the native core and keeps playback mutations ordered', async () => {
  ...
  const appBackgroundStart = player.indexOf('private func appBackground');
  ...
  assert.match(desktopLyrics, /systemName: "music\.note"/u);
  assert.match(desktopLyrics, /min\(52, max\(30, bounds\.height \* 0\.17\)\)/u);
  assert.match(desktopLyrics, /CACurrentMediaTime\(\)[\s\S]*truncatingRemainder\(dividingBy: 1\.18\)/u);
  ...
  assert.match(player, /private func appBackground\(for page: String\)/u);
  ...
  assert.match(themedTab, /appBackground\(for: page\)/u);
  assert.match(player, /ZStack \{\s+echoThemeBackground/u);
  assert.match(appBackground, /page == "control" \|\| playerModel\.appearanceBackground == "artwork"/u);
  ...
  assert.match(appBackground, /page != "control"[\s\S]*appearanceBackground == "custom"[\s\S]*appearanceImageUrl\.isEmpty/u);
  assert.match(player, /ZStack \{\s+echoWarmBackground\s+if !stableUrl\.isEmpty/u);
  assert.match(player, /@State private var stableIdentity = ""/u);
  assert.match(player, /@State private var stableUrl = ""/u);
  assert.doesNotMatch(player, /stableIdentity: \$stableArtworkIdentity/u);
